...
BUNDLE_OUT="$BUILD_DIR/Almaniac-1.0.16-signed.aab"
MAPPING_OUT="$BUILD_DIR/Almaniac-1.0.16-mapping.txt"
...
cp "$BUNDLE_OUT" "$BUILD_DIR/Almaniac-1.0.16-release.aab"

if [[ -f "$MAPPING_SRC" ]]; then
  cp "$MAPPING_SRC" "$MAPPING_OUT"
  mkdir -p "$ROOT/store/google-play"
  cp "$MAPPING_OUT" "$ROOT/store/google-play/Almaniac-1.0.16-mapping.txt"
else
  echo "Warning: mapping.txt not found at $MAPPING_SRC" >&2
fi

mkdir -p "$ROOT/store/google-play"
cp "$BUILD_DIR/Almaniac-1.0.16-release.aab" "$ROOT/store/google-play/Almaniac-1.0.16.aab"
